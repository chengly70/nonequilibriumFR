%script to test method on larger Nc=50
...
vr=1*.1; %vary this to 2*.1, 3*.1, 4*.1 to get different coupling STRENGTHS
...
mu_fn  = @(t) (2*(1-sin(2*pi*.5*t)).*exp(t./50))*ones(Nc,1);
...
% Save parameters at beginning
save dAn_n50a Nc Pstruct vr tmv mu_fn sig_fn
...
save dAn_n50sin_a covF_Ma covX_Ma mnX_Ma mnF_Ma Nc Pstruct vr tmv mu_fn sig_fn
